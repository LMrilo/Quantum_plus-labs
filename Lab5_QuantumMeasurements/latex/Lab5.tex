...
\newtcolorbox{reportbox}{colback=violet!5!white,colframe=violet!60!black,title=Report Requirement}
...
\noindent\textbf{Mode:} Online Simulators \quad\textbf{Credits:} 1 ECTS \quad\textbf{Deliverable:} PDF Report (10--15 pages, within 15 days)
...
The spin-$\frac{1}{2}$ observable along direction $\hat{n} = (\sin\theta\cos\varphi, \sin\theta\sin\varphi, \cos\theta)$ is:
\begin{equation}
    \hat{S}_{\hat{n}} = \frac{\hbar}{2}
    \bigl(\sin\theta\cos\varphi\,\hat{X} + \sin\theta\sin\varphi\,\hat{Y} + \cos\theta\,\hat{Z}\bigr)
    \label{eq:spin}
\end{equation}
The Stern-Gerlach apparatus oriented along $\hat{n}$ produces two spatially separated beams corresponding
to eigenvalues $+\hbar/2$ (spin-up) and $-\hbar/2$ (spin-down). This provides a direct physical
realization of quantum measurement.
...
\begin{reflectionbox}
\textbf{Question 7:} Express the Z$\to$X$\to$Z sequence using projection operators.
Let $\Pi_0^Z = |0\rangle\langle 0|$, $\Pi_+^X = |{+}\rangle\langle{+}|$.
Show that the probability of obtaining $0$ in the final Z measurement, starting from $|0\rangle$, is $\frac{1}{2}$.

\textbf{Question 8:} Why does the intermediate X measurement introduce irreducible randomness into
the final Z outcome, regardless of the initial state?
\end{reflectionbox}
...
\begin{reportbox}
Produce a \textbf{PDF report (10--15 pages)} documenting your work for each activity.
Submit within \textbf{15 days} of the lab session.
\end{reportbox}

\subsection*{Report Structure}

For each of the five activities, include:
\begin{enumerate}
    \item \textbf{Screenshots} of the simulator at key steps (minimum 2 per activity).
    \item A \textbf{measurement table} with columns:
          \textit{Initial State} $|$ \textit{Basis} $|$ \textit{Theoretical $P(m)$} (Born rule) $|$ \textit{Observed Outcome(s)}.
    \item A brief explanation (2--4 sentences) of each result in your own words.
\end{enumerate}

\subsection*{Reflection Questions to Answer in the Report}

Answer the following in clear, concise paragraphs (half a page each):

\begin{itemize}
    \item[\textbf{Q1}] \textit{``Why does measuring $|{+}\rangle$ in the X-basis always give the same result?''}\\
          Support with an explicit Born rule calculation: $|\langle{+}|\hat{X}|{+}\rangle|^2$.
    \item[\textbf{Q2}] \textit{``What happens to the Bloch vector after a Z-basis measurement?''}\\
          Describe the geometric transformation of an arbitrary equatorial state $|\psi\rangle$
          ($\theta = \pi/2$, arbitrary $\varphi$) under Z measurement.
    \item[\textbf{Q3}] \textit{``How does the Stern-Gerlach experiment demonstrate state collapse?''}\\
          Use the Z$\to$X$\to$Z sequence. Relate to the non-commutativity
          $[\hat{S}_x, \hat{S}_z] \neq 0$.
\end{itemize}
...
\begin{itemize}
    ...
    \item Course support material: \texttt{support\_material/quantum\_interference.pdf},
          \texttt{support\_material/phase\_kickback\_proof.pdf}.
