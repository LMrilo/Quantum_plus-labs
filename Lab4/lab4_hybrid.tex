...
\medskip
\textbf{OpenQASM 2.0 — sweep $\theta_0$, keep $\theta_1 = \pi$:}
\begin{lstlisting}[language={}]
OPENQASM 2.0;
include "qelib1.inc";
...
ry(pi) q[1];    // theta_1: keep at pi
cx q[0],q[1];
measure q[0] -> c[0];
measure q[1] -> c[1];
// Record probabilities. C(t0,pi) = 0.5*cos(t0) + cos(pi) = 0.5*cos(t0) - 1
// At t0=0:    C = -0.5 (|01> state)
// At t0=pi/2: C = -1.0
// At t0=pi:   C = -1.5 (ground state -- |10> state, the global minimum)
\end{lstlisting}
\end{studentbox}
