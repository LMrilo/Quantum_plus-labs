...
\newtcolorbox{bridgebox}{colback=purple!5!white,colframe=purple!70!black,
    title=Bridge to Lab 4 Finance: Portfolio Optimization, fonttitle=\bfseries}
...
Understanding this paradigm is a prerequisite for Lab 4 Finance, where you will apply
...
\begin{studentbox}
\textbf{Verify:} Set $\theta_0 = \pi$, $\theta_1 = \pi$. Show that
$\ket{\psi(\pi,\pi)} = \ket{10}$ and that $C(\pi, \pi) = -1.5 = E_0$.
\emph{Hint:} $R_y(\pi)\ket{0} = \ket{1}$ on both qubits gives $\ket{11}$,
and CX (control $q_0 = 1$) flips the target: $\ket{11} \to \ket{10}$.
\end{studentbox}
...
\begin{studentbox}
\textbf{Observe the parameter-shift rule directly:}
\begin{enumerate}
    \item Build the 1-qubit circuit: $R_y(\theta)\ket{0}$,
          measure $\langle Z\rangle$.
    \item Set $\theta = 0.5$ rad. Record $f(0.5)$.
    \item Change to $\theta = 0.5 + \pi/2$. Record $f^+$.
    \item Change to $\theta = 0.5 - \pi/2$. Record $f^-$.
    \item Compute $(f^+ - f^-)/2$. Compare with the analytic
          $-\sin(0.5) \approx -0.479$.
\end{enumerate}
\textbf{Expected:} The parameter-shift estimate matches the analytic
gradient exactly (within shot noise).
\end{studentbox}

\subsection{Exercise 2: Visualise the Cost Landscape}

\begin{studentbox}
\textbf{Two-qubit VQE landscape:}
\begin{enumerate}
    \item Build the ansatz $\mathrm{CX}_{01}(R_y(\theta_0)\otimes R_y(\theta_1))$.
    \item Fix $\theta_1 = \pi$. Sweep $\theta_0 \in \{0, \pi/4, \pi/2, 3\pi/4, \pi\}$.
    \item For each value, add $Z_0Z_1$ and $Z_0$ measurement and record
          $C(\theta_0, \pi)$ from the Probabilities panel.
    \item Plot by hand: along this line $C = -1 + \tfrac{1}{2}\cos\theta_0$, so you
          should see $C$ decrease monotonically from $-0.5$ (at $\theta_0=0$) to
          $-1.5$ (at $\theta_0=\pi$), the global minimum.
\end{enumerate}
\end{studentbox}
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Part G: Bridge to Lab 4 Finance and Assessment}
% ══════════════════════════════════════════════════════════════════════════════

\begin{bridgebox}
\textbf{From MaxCut to Portfolio Optimization}

In Lab 4 Finance, the portfolio optimization problem is formulated as:
\[
