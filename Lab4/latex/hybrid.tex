...
\begin{enumerate}
    ...
    \item Fix $\theta_1 = 0$. Sweep $\theta_0 \in \{0, \pi/4, \pi/2, 3\pi/4, \pi\}$.
    \item For each value, add $Z_0Z_1$ and $Z_0$ measurement and record
          $C(\theta_0, 0)$ from the Probabilities panel.
    \item Plot by hand: you should see $C$ decrease monotonically from $1.5$
          (at $\theta_0=0$) to $-1.5$ (at $\theta_0=\pi$).
\end{enumerate}
\end{studentbox}
