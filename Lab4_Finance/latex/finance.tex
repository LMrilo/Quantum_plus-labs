...
\begin{studentbox}
\textbf{Visualise the Portfolio Hamiltonian Structure:}
\begin{enumerate}
    \item Build a 4-qubit circuit with $H$ on all qubits.
    \item Add the first $ZZ$ term of $H_\text{portfolio}$
          (AAPL--MSFT, coefficient $+3.09$):
          CNOT$(q_0, q_1)$ --- $R_z(2\gamma \cdot 3.09)$ on $q_1$
          --- CNOT$(q_0, q_1)$, with $\gamma = 0.5$.
    \item Add the $Z_0$ single-qubit term ($+0.157$):
          $R_z(2\gamma \cdot 0.157)$ on $q_0$.
    \item Apply $R_x(2\beta)$ on all four qubits with $\beta = 0.8$.
    \item Measure and observe the Q-Sphere. The states corresponding
          to feasible 2-asset portfolios (exactly two 1s) should
          dominate.
\end{enumerate}
\textbf{Observation:} Even one QAOA layer begins to suppress infeasible
states. Compare the Q-Sphere with and without the $R_z$ terms.
\end{studentbox}
