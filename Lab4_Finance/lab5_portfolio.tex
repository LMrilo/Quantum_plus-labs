...
Expand the budget penalty (dropping the constant $K^2$):
\[
...
\end{itemize}
Convention note: the symmetric matrix entry $Q_{ij}$ ($i\neq j$) in
equation~\eqref{eq:Q} is defined as $\lambda\Sigma_{ij}+2\lambda_\text{pen}$
so that the full product $x^TQx = \sum_{i,j}Q_{ij}x_ix_j$ (which counts each
pair twice) equals the financial cost plus a constant shift
$-\lambda_\text{pen}K$ for all feasible portfolios.
The shift is the same for every feasible $x$, so it does not affect the ranking.
\end{warningbox}
...
\[
    Q = \begin{pmatrix}
        -\crd{9.2559} & 6.0588 & 6.0313 & 6.0203 \\
        6.0588 & -\crd{9.1971} & 6.0272 & 6.0170 \\
        6.0313 & 6.0272 & -\crd{9.0924} & 6.0294 \\
        6.0203 & 6.0170 & 6.0294 & -\crd{9.0975}
    \end{pmatrix}.
\]
...
\begin{align}
    ...
    \underbrace{\bigl(I - Z_i - Z_j + Z_iZ_j\bigr)}_{\text{expand: }(1-Z_i)(1-Z_j)}
    \notag\\
...
\begin{tabular}{lrl}
    ...
    \midrule
    $Z_0 Z_1$ & $+3.0294$ & AAPL--MSFT: $Q_{01}/2 = 6.0588/2$ \\
    $Z_0 Z_2$ & $+3.0157$ & AAPL--JPM:  $Q_{02}/2 = 6.0313/2$ \\
    $Z_0 Z_3$ & $+3.0102$ & AAPL--XOM:  $Q_{03}/2 = 6.0203/2$ \\
    $Z_1 Z_2$ & $+3.0136$ & MSFT--JPM:  $Q_{12}/2 = 6.0272/2$ \\
    $Z_1 Z_3$ & $+3.0085$ & MSFT--XOM:  $Q_{13}/2 = 6.0170/2$ \\
    $Z_2 Z_3$ & $+3.0147$ & JPM--XOM:   $Q_{23}/2 = 6.0294/2$ \\
    $Z_0$     & \crd{?} & $h_0 = -\tfrac{1}{2}\sum_j Q_{0j}$ — see note below \\
    $Z_1$     & \crd{?} & $h_1 = -\tfrac{1}{2}\sum_j Q_{1j}$ — see note below \\
    $Z_2$     & \crd{?} & $h_2 = -\tfrac{1}{2}\sum_j Q_{2j}$ — see note below \\
    $Z_3$     & \crd{?} & $h_3 = -\tfrac{1}{2}\sum_j Q_{3j}$ — see note below \\
    \bottomrule
\end{tabular}
\end{center}

\begin{warningbox}[Computing single-qubit $Z$ coefficients]
The $Z_i$ coefficient is $h_i = -\tfrac{1}{2}\sum_{j=0}^{3} Q_{ij}$.
Using the corrected $Q$ matrix above:
\[
h_0 = -\tfrac{1}{2}(-9.2559 + 6.0588 + 6.0313 + 6.0203) = -\tfrac{1}{2}(8.8545) = -4.4273
\]
Similarly for $h_1,h_2,h_3$.  These large negative values arise because the dominant
contribution is the off-diagonal penalty ($2\lambda_\text{pen} = 6$ per pair times 3
pairs = $+9$ per row, partially offset by the diagonal $\approx -9.2$).
The Colab notebook computes these coefficients directly via
\texttt{SparsePauliOp.from\_list} from the $Q$ matrix; students should verify
the code output against these formulas rather than the values in a table.
\end{warningbox}
...
the state $\ket{0011}$ (q$_0$=AAPL=1, q$_1$=MSFT=1, q$_2$=JPM=0,
q$_3$=XOM=0) should have the highest probability.

\begin{warningbox}[Qiskit Bitstring Convention]
Qiskit uses \emph{little-endian} ordering: the rightmost bit in the
output string is qubit 0. Qubit assignment: q$_0$=AAPL, q$_1$=MSFT,
q$_2$=JPM, q$_3$=XOM. Therefore the Qiskit bitstring
$\ket{q_3 q_2 q_1 q_0}$ for ``AAPL and MSFT selected'' is
$\ket{0011}$, \emph{not} $\ket{1100}$. The portfolio vector notation
$x = [x_0, x_1, x_2, x_3] = [1,1,0,0]$ uses the opposite (left-to-right)
order and appears in the tables above; do not confuse the two.
\end{warningbox}

\begin{finbox}[Technical $\to$ Business Translation (Qiskit bitstrings)]
\begin{center}
\begin{tabular}{cll}
    \toprule
    Qiskit bitstring $\ket{q_3 q_2 q_1 q_0}$ & Portfolio vector $x$ & Business meaning \\
    \midrule
    $\ket{0011}$ & $[1,1,0,0]$ & Invest in AAPL and MSFT \\
    $\ket{1001}$ & $[1,0,0,1]$ & Invest in AAPL and XOM \\
    $\ket{1100}$ & $[0,0,1,1]$ & Invest in JPM and XOM \\
    $\ket{0000}$ & $[0,0,0,0]$ & Infeasible (no assets) \\
    $\ket{1111}$ & $[1,1,1,1]$ & Infeasible (all assets, $K \neq 2$) \\
    \bottomrule
\end{tabular}
\end{center}
...
\emph{success probability} $P^* = P(\ket{0011})$: the probability
